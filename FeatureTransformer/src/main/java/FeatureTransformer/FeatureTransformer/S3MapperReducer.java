...
public class S3MapperReducer {
		...
		public void map(Object key, Text value, Context context) {
			...
			try {
				...
				if (extention.toLowerCase().equals("snappy")) {
					System.out.printf("extention = %s, process snappy file\n", extention);
					...
					System.out.printf("extention = %s, process json file\n", extention);
					loanJson = AWSS3Client.getInstance().getJsonFromS3Json(inputBucketName, inputObjKey);
				}

				System.out.printf("The json for S3 object %s is: \n", inputObjKey);
				System.out.println(loanJson);
				System.out.println("\n");
				AWSS3Client.getInstance().putS3Object(outputPath, inputObjKey, loanJson);
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
		...
		if (otherArgs.length != 2) {
			System.err.println("Usage: S3MapperReducer<input_bucket_name><output_bucket_name");
			System.exit(2);
		}
		...
		String inputBucketName = AWSS3Client.getInstance().getBucketAndPrefix(inputPath)[0];
		String inputPrefix = AWSS3Client.getInstance().getBucketAndPrefix(inputPath)[1];

		String outputBucketName = AWSS3Client.getInstance().getBucketAndPrefix(outputPath)[0];
		String outputPrefix = AWSS3Client.getInstance().getBucketAndPrefix(outputPath)[1];
		...
		//FileInputFormat.addInputPath(job, new Path(tempKey ));		//run on loca
